...
export function readVercelRequestContext(request) {
  ...
  return {
    ...
    // --- True visitor geolocation (the whole point of this helper) ---
    // Country and city ONLY. Vercel also exposes
    // x-vercel-ip-latitude / -longitude / -postal-code / -timezone /
    // -country-region / -continent, and this helper used to collect them all.
    // It no longer does, for two reasons:
    //
    //   1. Nothing in the dashboard displays them. They were collected,
    //      transported, written to your application logs, and then discarded by
    //      lib/privacy.js. Pure exposure with no benefit.
    //   2. Street-level coordinates and a postal code identify a household far
    //      more precisely than a city name, and once logged they also live in
    //      Vercel's own log retention — outside this app's control.
    //
    // Not collecting beats collecting-then-stripping: there is no window in
    // which the value exists, and nothing depends on the scrubbing working.
    // Add a field back only if something actually renders it.
