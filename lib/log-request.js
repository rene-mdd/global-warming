...
export function readVercelRequestContext(request) {
  ...
  try {
    ...
    path = request?.url ? new URL(request.url).pathname + (new URL(request.url).search || "") : undefined;
  } catch {
  ...
  return {
    ...
    // Which Vercel regions the request passed through / executed in.
    vercelId: get("x-vercel-id"),
    deploymentUrl: get("x-vercel-deployment-url"),
  };
}
