...
/**
 * Write (merge) one day's rollup summary. Shallow-merged at the top level,
 * not replaced outright: two independent writers share a day's record —
 ...
 * totalTrafficRequests/totalTrafficBreakdown separately, on its own schedule.
 * A blind overwrite would let whichever runs second silently erase the
 * other's fields for that date. A re-run/backfill of one writer still
 * overwrites its OWN fields, which is the intended "re-run wins" behavior —
 * it just no longer takes the other writer's fields down with it.
 * The whole map is small — at DAILY_MAX_DAYS ~= 400 days of ~1KB summaries
 * this is still well under a megabyte — so rewriting the file whole each
 * time is fine, unlike the append-then-occasionally-rewrite dance the events
 * log needs.
 */
