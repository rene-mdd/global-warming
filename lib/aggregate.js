...
import { mergeRequestRecords } from "./drain-parse.js";
...
function topBreakdown(records, getKey, { limit = 8, includeOther = true } = {}) {
  ...
  if (tailCount > 0) head.push({ key: "Other", count: tailCount, isOther: true });
  return head;
}
...
export function aggregate(records, { startTime, endTime }) {
  ...
  for (let t = firstBucket; t <= endTime; t += bucketMs) {
    buckets.set(t, { t, total: 0, s2xx: 0, s3xx: 0, s4xx: 0, s5xx: 0, other: 0 });
  }
  ...
  return {
    ...
    breakdowns: {
      ...
      clientIp: topBreakdown(inWindow, (r) => r.clientIp, { limit: 8, includeOther: false }),
    ...
    statusClasses: [
      { key: "s2xx", label: "2xx success", count: series.reduce((s, b) => s + b.s2xx, 0) },
      { key: "s3xx", label: "3xx redirect", count: series.reduce((s, b) => s + b.s3xx, 0) },
      { key: "s4xx", label: "4xx client error", count: series.reduce((s, b) => s + b.s4xx, 0) },
      { key: "s5xx", label: "5xx server error", count: series.reduce((s, b) => s + b.s5xx, 0) },
    ],
  };
}
...
export function aggregateLocations(
  ...
  for (const record of inWindow) {
    ...
    if (record.city) {
      const city = entry.cityMap.get(record.city) ?? { city: record.city, requests: 0, ips: new Set() };
      city.requests += 1;
    ...
    if (ip) {
      ...
      if (!existing) {
        ...
        if (record.timestamp < existing.firstSeen) existing.firstSeen = record.timestamp;
        ...
        if (record.route) {
          existing.routes.set(record.route, (existing.routes.get(record.route) ?? 0) + 1);
        }
      }
    }
  }

  const sorted = [...countries.values()].sort((a, b) => b.requests - a.requests);
  const shownCountries = sorted.slice(0, limitCountries);

  const result = shownCountries.map((entry) => {
    const ips = [...entry.ipMap.values()].sort((a, b) => b.requests - a.requests);
    ...
    return {
      ...
      geoQuality:
        entry.approxCount === 0 ? "accurate" : entry.accurateCount === 0 ? "approximate" : "mixed",
      cities: [...entry.cityMap.values()]
        .map((c) => ({ city: c.city, requests: c.requests, uniqueIps: c.ips.size }))
