...
/**
 * Collapses log entries into one row per HTTP request, merging entries that
 * share a `requestId`. Entries with `isRequest` false (e.g. build output) are
 * excluded.
 */
...
export function aggregate(records, { startTime, endTime }) {
  ...
  let withCountry = 0;
  let botRequests = 0;
  // Number of records whose clientIp is not anonymised.
  let unanonymised = 0;
  ...
  for (const record of inWindow) {
    ...
    if (record.country) withCountry += 1;
  ...
  return {
    ...
    totals: {
      ...
      requests,
      // Raw log entries received, normally higher than `requests`.
      ...
      bytes,
      botRequests,
      // How many records carry a country. Only the instrumented public API
      // routes populate this (see lib/log-request.js).
      withCountry,
      countryCoverage: requests ? withCountry / requests : 0,
...
/**
 * Builds a country -> IP tree of request activity, with its own
 * `limitCountries`/`limitIpsPerCountry` caps, served from a separate
 * function and endpoint from aggregate(). Records are merged by requestId
 * first, same as aggregate(). Truncation from the caps is reported back as
 * `countriesTruncated`/`ipsTruncated` rather than applied silently.
 */
