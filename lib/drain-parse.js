...
//
// 1. BODY FORMAT. A drain can be configured as "JSON" or "NDJSON", and Vercel
//    batches multiple log events per request. In practice you may receive a
//    JSON array, a single JSON object, or newline-delimited JSON objects. We
//    sniff all three rather than trusting the dashboard setting.
//
// 2. WHERE EACH FIELD LIVES. Request-level data (client IP, method, user agent,
...
import { regionInfo } from "./vercel-regions.js";
...
export function normalizeEvent(raw) {
  ...
  const timestamp = Number(firstDefined(proxy.timestamp, raw.timestamp, Date.now()));
  ...
  const statusCodeRaw = firstDefined(proxy.statusCode, raw.statusCode, custom?.statusCode);
  const statusCode = statusCodeRaw === undefined ? undefined : Number(statusCodeRaw);
  ...
  // --- Geo. True visitor country only exists if it was logged from the
  // function via geo headers; otherwise approximate from the edge region. ---
  const edgeRegion = firstDefined(proxy.region, raw.executionRegion, proxy.lambdaRegion);
  const headerCountry = firstDefined(custom?.country, custom?.geo?.country);
  const headerCity = firstDefined(custom?.city, custom?.geo?.city);

  let country = headerCountry;
  let city = headerCity;
  let geoSource = headerCountry ? "headers" : null;

  if (!country && edgeRegion) {
    const info = regionInfo(edgeRegion);
    if (info) {
      country = info.country;
      city = city ?? info.city;
      geoSource = "edge-region";
    }
  }
  ...
  const record = {
    ...
    responseByteSize:
      proxy.responseByteSize === undefined ? undefined : Number(proxy.responseByteSize),
    ...
    country,
    city,
    countryRegion: firstDefined(custom?.countryRegion, custom?.geo?.countryRegion),
    latitude: firstDefined(custom?.latitude, custom?.geo?.latitude),
    longitude: firstDefined(custom?.longitude, custom?.geo?.longitude),
    timezone: firstDefined(custom?.timezone, custom?.geo?.timezone),
    geoSource,
    ...
    message: typeof raw.message === "string" ? raw.message.slice(0, 2000) : undefined,
 ...
 *
 * Preference rules: a defined value beats undefined; header-derived geo beats
 * an edge-region approximation; custom objects are shallow-merged.
 */
export function mergeRequestRecords(a, b) {
  ...
  for (const [key, value] of Object.entries(b)) {
    ...
    if (key === "custom") continue; // handled below
    if (merged[key] === undefined || merged[key] === null || merged[key] === "") {
      merged[key] = value;
    }
  }

  // True geolocation always wins over the edge-region guess.
  if (b.geoSource === "headers" && a.geoSource !== "headers") {
    merged.country = b.country;
    merged.city = b.city ?? a.city;
    merged.countryRegion = b.countryRegion ?? a.countryRegion;
    merged.latitude = b.latitude ?? a.latitude;
    merged.longitude = b.longitude ?? a.longitude;
    merged.timezone = b.timezone ?? a.timezone;
    merged.geoSource = "headers";
  }
