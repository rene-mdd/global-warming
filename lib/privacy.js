...
//
// Data minimisation for the personal data this app necessarily handles.
//
// IP addresses are personal data under the GDPR — the CJEU settled that even
// dynamic IPs count when they can be combined with other data to identify
// someone (Breyer, C-582/14). This app also stores city-level geolocation and
// user agents. That's fine if you need it and handle it lawfully, but the
// cheapest risk reduction is to not store the raw value in the first place.
//
// Anonymisation runs at INGEST, before anything touches disk, so raw IPs are
// never persisted when it's enabled.
//
// Modes (DRAIN_ANONYMIZE_IPS):
//   off       store the IP as received (default — you opt in to privacy)
//   truncate  zero the host part: 203.0.113.47 -> 203.0.113.0, IPv6 -> /48
//   hash      salted SHA-256, first 16 hex chars: "a3f1c8d92b04e7f5"
//   drop      discard the IP entirely
//
// Trade-offs, honestly:
//   - `truncate` is irreversible and keeps rough network grouping, but "unique
//     IPs" becomes "unique /24 subnets", which over-counts shared networks and
//     under-counts users behind different subnets.
//   - `hash` keeps per-visitor uniqueness exact. It is pseudonymisation, NOT
//     anonymisation: IPv4 is only ~4.3 billion values, so an attacker with the
//     salt can brute-force the whole space in minutes. It still meaningfully
//     limits exposure if your store leaks WITHOUT the salt. Keep the salt out
//     of the same store, and rotate it periodically (rotating resets
//     unique-visitor continuity, which is the point).
//   - `drop` is the only option with zero IP risk. Unique-visitor counts stop
//     working; everything else still does.

import crypto from "crypto";
import { TRAFFIC_LOG_MARKER } from "./drain-parse.js";
...
const IPV4_IN_TEXT = /(?<![\w./])((?:25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(?:\.(?:25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3})(?![\w.])/g;

// Conservative IPv6: at least four colon-separated hex groups.
const IPV6_IN_TEXT = /(?<![\w:])((?:[0-9a-fA-F]{1,4}:){3,7}[0-9a-fA-F]{1,4})(?![\w:])/g;
...
export function applyPrivacy(record, mode = anonymizeMode()) {
  ...
  if (scrubbed.custom && typeof scrubbed.custom === "object") {
    ...
    for (const key of ["clientIp", "ip", "latitude", "longitude", "postalCode"]) {
    ...
    if (typeof scrubbed.message === "string" && scrubbed.message.includes("{")) {
