...
function ipSalt() {
  ...
  // This throws rather than silently degrading to truncation, so a
  // misconfigured deployment fails loudly (see pages/api/drains/ingest.js)
  // instead of quietly storing weaker-than-requested anonymisation.
  throw new Error(
 ...
 *
 * Precise coordinates are dropped for anything stronger than `off` as
 * defense-in-depth — the record shape produced upstream (lib/drain-parse.js)
 * no longer includes latitude/longitude or city at all, only country, so
 * these deletes are normally no-ops rather than a sign those fields are
 * expected to arrive.
 */
