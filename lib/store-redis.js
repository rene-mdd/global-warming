...
//
// Upstash Redis backend for the drain store. Use this when the app runs
// somewhere without a persistent filesystem — i.e. on Vercel.
//
// ---------------------------------------------------------------------------
// WHY A SORTED SET AND NOT A LIST
// ---------------------------------------------------------------------------
// Events are stored in one ZSET, scored by their timestamp. That gives three
// things a plain list can't:
//
//   1. Reads filter server-side. `ZRANGEBYSCORE key <since> +inf` returns only
//      the window you asked for, instead of shipping all 20,000 events over the
//      network so JavaScript can throw most of them away.
//   2. Retention actually DELETES. `ZREMRANGEBYSCORE key -inf <cutoff>` removes
//      expired records. (An earlier sketch of this file filtered by age on read,
//      which left expired personal data sitting in the database indefinitely —
//      invisible, but still stored. That's not a retention policy.)
//   3. Redelivery is idempotent for free. ZSET members are unique, so if Vercel
//      retries a delivery the same event lands on the same member and doesn't
//      double-count.
//
// The trade-off: two identical events genuinely emitted twice would collapse
// into one. In practice every Vercel log entry carries a distinct `id`, so this
// only ever merges true duplicates.
