...
// Traction over longer periods comes from the drain:daily rollup below, which
// is ~1000x smaller per day and doesn't compete with this cap.
...
/**
 * Write (merge) one day's rollup summary. A plain HSET, not a ZADD: there's
 * one value per calendar day, so a hash keyed by date string is the natural
 * shape and lets a re-run (backfill, a fixed cron) simply overwrite its own
 * fields.
 *
 * Shallow-merged with whatever is already stored for that date, not blindly
 * replaced: two independent writers share a day's record —
 * pages/api/drains/rollup.js writes the drain-derived fields (requests,
 * uniqueVisitors, ...) nightly, and pages/api/drains/traffic-total.js writes
 * totalTrafficRequests/totalTrafficBreakdown separately, on its own
 * schedule. Whichever ran second would otherwise silently erase the other's
 * fields for that date. The HGET-then-HSET here is a small, accepted race
 * (two writers for the same date within the same instant) — at one write per
 * writer per day the odds are negligible, and it costs one extra read per
 * call, not per record.
 */
export async function writeDailySummary(dateKey, summary) {
  ...
  // Bound the hash the same way the event ZSET is bounded, just by field count
  // instead of rank: list the dates, drop the oldest past the cap. At
  // DAILY_MAX_DAYS ~= 400 this is at most a few hundred field names, nowhere
  // near the per-request size limit that matters for drain:events.
