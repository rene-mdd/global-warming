...
/**
 * Write one day's rollup summary. A plain HSET, not a ZADD: there's one value
 * per calendar day, so a hash keyed by date string is the natural shape and
 * lets a re-run (backfill, a fixed cron) simply overwrite that field.
 */
export async function writeDailySummary(dateKey, summary) {
  const redis = await getClient();
  await redis.hset(DAILY_KEY, { [dateKey]: JSON.stringify(summary) });
