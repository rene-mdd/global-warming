...
// Returns every record with score >= min, fetched as parallel bounded
// ZRANGE...BYSCORE pages (order not guaranteed — callers don't rely on it).
// The +1 page covers writes landing between the ZCOUNT and the ZRANGE calls.
async function readAllByScore(redis, min) {
  const total = await redis.zcount(KEY, min, "+inf");
  if (!total) return [];

  const pages = Math.ceil(total / READ_PAGE_SIZE) + 1;
  const pageResults = await Promise.all(
    Array.from({ length: pages }, (_, i) =>
      redis.zrange(KEY, min, "+inf", {
        byScore: true,
        offset: i * READ_PAGE_SIZE,
...
// Short-lived memo for unbounded reads, shared by /stats and /locations
// (both read the same window). Not applied to bounded (`limit` set) reads —
// that's the live events table, which shouldn't lag behind Redis.
const READ_CACHE_MS = Number(process.env.DRAIN_READ_CACHE_MS ?? 15000);
let readCache = null; // { bucket, promise }

function cachedReadAllByScore(redis, min) {
  if (READ_CACHE_MS <= 0) return readAllByScore(redis, min);

  const bucket = Math.floor(min / READ_CACHE_MS);
  if (readCache?.bucket === bucket) return readCache.promise;

  const promise = readAllByScore(redis, min).catch((err) => {
    // Don't cache a failure — let the next call retry against Redis.
    readCache = null;
    throw err;
  });
  readCache = { bucket, promise };
  return promise;
}
...
export async function readRecords({ since, limit } = {}) {
  ...
  if (!bounded) {
    // With no `limit`, reads the whole window — see cachedReadAllByScore.
    return cachedReadAllByScore(redis, min);
  }
...
export async function clearRecords() {
  ...
  await redis.del(KEY);
  // Otherwise a just-cleared store could still serve stale pre-clear data to
  // an aggregation read for up to DRAIN_READ_CACHE_MS.
  readCache = null;
  return had ?? 0;
}
