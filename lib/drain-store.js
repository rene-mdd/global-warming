...
// Meanwhile the file backend is genuinely nicer for local development (no
// service to run, `npm run seed` just works). Detecting rather than choosing
// means you get the right one in both places.
//
// Force a backend with DRAIN_STORE=redis | file if you ever need to.
//
// ---------------------------------------------------------------------------
// THE INTERFACE
// ---------------------------------------------------------------------------
...
//     `limit` returns only the newest N records, bounding the size of the read.
//     Use it for feeds; omit it for aggregations that need the whole window.
...
//     separately from the bounded event window above so "how much traffic
//     over the last month" doesn't compete with DRAIN_MAX_EVENTS/RETENTION.
//     Shallow-merges `summary` into whatever's already stored for that date
//     — pages/api/drains/rollup.js (drain-derived fields) and
//     pages/api/drains/traffic-total.js (totalTrafficRequests, from Vercel's
//     own metrics) both write to the same day's record on independent
//     schedules, and neither should erase the other's fields.
