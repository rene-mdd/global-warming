...
const convertToObject = valueArray2d => valueArray2d.map(([time, station, land]) => {
    return {time, station, land};
});

export default async (req, res) => {
    try {
        const {data} = await axios.get('https://data.giss.nasa.gov/gistemp/graphs_v4/graph_data/Monthly_Mean_Global_Surface_Temperature/graph.txt')
