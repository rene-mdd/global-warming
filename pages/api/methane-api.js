...
export default async (req, res) => {
    ...
    const {data, error} = await getFTPData(connectionConfig, 'products/trends/ch4/ch4_mm_gl.txt');
    ...
    function parsedData(csvToJson) {
        ...
        sliced.forEach(obj => {
            if (oldKey !== 'year') {
                Object.defineProperty(obj, ['year'], Object.getOwnPropertyDescriptor(obj, oldKey));
                delete obj[oldKey];
            }
            methaneData.push({
                date: `${
                    obj.year.split(' ').filter(f => f)[0]
                }.${
                    obj.year.split(' ').filter(f => f)[1]
                }`,
                average: `${
                    obj.year.split(' ').filter(f => f)[3]
                }`,
                trend: `${
                    obj.year.split(' ').filter(f => f)[5]
                }`,
                averageUnc: `${
                    obj.year.split(' ').filter(f => f)[4]
                }`,
                trendUnc: `${
                    obj.year.split(' ').filter(f => f)[6]
                }`
            });
        });
        // cors config
...
const getFTPData = (config, path) => { // Creating FTP Client
