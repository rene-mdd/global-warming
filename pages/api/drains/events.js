// pages/api/drains/events.js   — Pages Router version
//
// Raw event feed for the live table.
//   GET /api/drains/events?limit=150&hours=24&q=/api&status=5xx
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (statusFilter) {
      const wanted = statusFilter.startsWith("s") ? statusFilter : `s${statusFilter}`;
      records = records.filter((r) => statusClass(r.statusCode) === wanted);
    }

    if (query) {
      records = records.filter((r) =>
        [r.path, r.clientIp, r.country, r.city, r.host, r.userAgent, r.method, r.message]
          .filter(Boolean)
    ...
    return res.status(200).json({ events: page, total: records.length, limit, hours });
  } catch (err) {
