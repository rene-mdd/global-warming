// pages/api/drains/stats.js   — Pages Router version
//
...
export default async function handler(req, res) {
  ...
  // Returns visitor data, so it refuses to run unauthenticated in production.
  const refusal = checkApiAuth(req);
  if (refusal) return res.status(refusal.status).json(refusal.body);

  // Pages Router gives you parsed query params directly — no URL parsing needed.
